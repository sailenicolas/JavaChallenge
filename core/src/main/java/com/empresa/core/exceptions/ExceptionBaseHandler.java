...
public class ExceptionBaseHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handlerWebClientResponseException(WebClientResponseException ex){
        logger.error("Error {} ", ExceptionUtils.getRootCauseMessage(ex));
    ...
    public ResponseEntity<Object> handlerWebClientResponseException(WebClientRequestException ex){
        logger.error("Error {} ", ExceptionUtils.getRootCauseMessage(ex));
    ...
    protected Mono<ResponseEntity<Object>> handleWebExchangeBindException(WebExchangeBindException ex, HttpHeaders headers, HttpStatusCode status, ServerWebExchange exchange) {
        ...
        return Mono.just(ResponseEntity.status(status)
                .contentType(MediaType.APPLICATION_JSON)
                .body(new ApiError(status, ex.getFieldErrors().parallelStream().map(o -> "%s: %s - %s".formatted(o.getField(), "failed", o.getDefaultMessage())).toList())));
    }
    ...
    public Mono<ResponseEntity<ApiError>> handlerException(Exception ex, ServerWebExchange exchange) {
        logger.error("Error {} ", ExceptionUtils.getRootCauseMessage(ex));
